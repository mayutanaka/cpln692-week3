(function(){
  ...
  _.each(schools, function(school) {
    ...
    if (typeof school.GRADE_ORG === 'number') {
      ...
      var level = school.GRADE_LEVEL.toUpperCase();
      school.HAS_KINDERGARTEN = level.includes('K');
      school.HAS_ELEMENTARY = level.includes('ELEM');
      school.HAS_MIDDLE_SCHOOL = level.includes('MID');
      school.HAS_HIGH_SCHOOL = level.includes('HIGH');
    }
  });
  ...
  // filter data
  function validate(s) {
    open = s.ACTIVE.toUpperCase() == 'OPEN';
    sch = s.HAS_KINDERGARTEN || s.HAS_ELEMENTARY || s.HAS_MIDDLE_SCHOOL || s.HAS_HIGH_SCHOOL;
    pub = s.TYPE.toUpperCase()!=='CHARTER' || s.TYPE.toUpperCase()!=='PRIVATE';
    enr = s.ENROLLMENT > minEnrollment;
    zip = _.contains(acceptedZipcodes, s.ZIPCODE);
    return open && sch && pub && enr && !zip;
  }
  var filtered_data = _.filter(schools, validate);
  var filtered_out = _.filter(schools, function(s){ return !validate(s); });
  ...
  _.each(filtered_data, function(d) {
    ...
    var pathOpts = {'radius': d.ENROLLMENT/30,
                    'fillColor': color};
    L.circleMarker([d.Y, d.X], pathOpts).bindPopup(d.FACILNAME_LABEL).addTo(map);
  });
